...
/* DdsFileSubscriber.java

A publication of data of type DdsFile
...
Example publication of type DdsFile automatically generated by 
...
java DdsFileSubscriber <domain_id> <sample_count>

(3) Start the publication on the same domain used for RTI Data Distribution
Service with the command
java DdsFilePublisher <domain_id> <sample_count>
...
java -Djava.ext.dirs=$NDDSHOME/class DdsFilePublisher <domain_id>

java -Djava.ext.dirs=$NDDSHOME/class DdsFileSubscriber <domain_id>  
*/
...
public class DdsFileSubscriber {
	// -----------------------------------------------------------------------
	// Public Methods
	// -----------------------------------------------------------------------
	...
	private static void subscriberMain(int domainId, int sampleCount) {
		...
		try {
			...
			topic = participant.create_topic("Example DdsFile", typeName, DomainParticipant.TOPIC_QOS_DEFAULT,
			...
			reader = (DdsFileDataReader) subscriber.create_datareader(topic, Subscriber.DATAREADER_QOS_DEFAULT,
			...
			for (int count = 0; (sampleCount == 0) || (count < sampleCount); ++count) {
				System.out.println("DdsFile subscriber sleeping for " + receivePeriodSec + " sec...");
	...
	private static class DdsFileListener extends DataReaderAdapter {

		DdsFileSeq _dataSeq = new DdsFileSeq();
		SampleInfoSeq _infoSeq = new SampleInfoSeq();

		public void on_data_available(DataReader reader) {
			DdsFileDataReader DdsFileReader = (DdsFileDataReader) reader;

			try {
				DdsFileReader.take(_dataSeq, _infoSeq, ResourceLimitsQosPolicy.LENGTH_UNLIMITED,
				...
				for (int i = 0; i < _dataSeq.size(); ++i) {
					...
					if (info.valid_data) {
						System.out.println(((DdsFile) _dataSeq.get(i)).toString("Received", 0));
				...
				DdsFileReader.return_loan(_dataSeq, _infoSeq);
			}
		}
	}
}
