...
java -Djava.ext.dirs=$NDDSHOME/lib/java DdsFilePublisher <domain_id>

java -Djava.ext.dirs=$NDDSHOME/lib/java DdsFileSubscriber <domain_id>        
*/
...
public class DdsFilePublisher {
	...
	private static void publisherMain(int domainId, int sampleCount) {
		...
		try {
			...
			writer = (DdsFileDataWriter) publisher.create_datawriter(topic, Publisher.DATAWRITER_QOS_DEFAULT,
					null /* listener */, StatusKind.STATUS_MASK_NONE);
			if (writer == null) {
			...
			for (int count = 0; (sampleCount == 0) || (count < sampleCount); ++count) {
				System.out.println("Writing DdsFile, count " + count);
			...
			/*
			 * RTI Data Distribution Service provides finalize_instance() method for people
			 * who want to release memory used by the participant factory singleton.
