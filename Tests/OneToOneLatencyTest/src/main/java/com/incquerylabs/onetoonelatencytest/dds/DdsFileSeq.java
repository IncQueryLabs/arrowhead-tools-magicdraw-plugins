...
public final class DdsFileSeq extends LoanableSequence implements Copyable {
	...
	/**
	 * When a memory loan has been taken out in the lower layers of RTI Data
	 * Distribution Service, store a pointer to the native sequence here. That way,
	 * when we call finish(), we can give the memory back.
	 */
	...
	public DdsFileSeq(int initialMaximum) {
		super(DdsFile.class, initialMaximum);
	}

	@SuppressWarnings("rawtypes")
	public DdsFileSeq(Collection elements) {
		super(DdsFile.class, elements);
	}

	public DdsFile get(int index) {
		return (DdsFile) super.get(index);
	}

	// --- From Copyable: ----------------------------------------------------

	/**
	 * Copy data into <code>this</code> object from another. The result of this
	 * method is that both <code>this</code> and <code>src</code> will be the same
	 * size and contain the same data.
	 * 
	 * @param src The Object which contains the data to be copied
	 * @return <code>this</code>
	 * @exception NullPointerException If <code>src</code> is null.
	 * @exception ClassCastException   If <code>src</code> is not a
	 *                                 <code>Sequence</code> OR if one of the
	 *                                 objects contained in the
	 *                                 <code>Sequence</code> is not of the expected
	 *                                 type.
	...
	public Object copy_from(Object src) {
		...
		for (int i = 0; (i < origSize) && (i < srcSize); i++) {
			if (typedSrc.get(i) == null) {
				...
				// check to see if our entry is null, if it is, a new instance has to be
				// allocated
				if (get(i) == null) {

					set(i, DdsFile.create());
				}
				set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
			}
		}

		// copy 'new' DdsFile objects (beyond the original size of this object)
